...
/*
 * Keep the accepted Stage 1 FMAN/PCD bootstrap active. The ask-dpa-app
 * userspace helper populates CDX with the port and table state that later
 * fragment/reassembly setup depends on.
 */
...
static int start_dpa_app(void)
{
	...
	/*
	 * Keep a serial-readable trace of ask-dpa-app failures during bring-up.
	 * The wrapper execs the real binary and logs its stdout/stderr to /tmp.
	 */
	static char *modprobe_path = "/usr/bin/dpa_app_wrapper";
